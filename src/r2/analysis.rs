...
pub unsafe fn get_all_functions(core: *mut RCore) -> Vec<u64> {
    ...
    let Some(json) = cmd_json(core, "aflj") else { return functions };
    for func in json.as_array().unwrap_or(&Vec::new()) {
        if let Some(addr) = func.get("offset").or_else(|| func.get("addr")).and_then(|v| v.as_u64()) {
            functions.push(addr);
        }
    }
    functions
...
pub unsafe fn get_function_at(core: *mut RCore, addr: u64) -> Option<FunctionInfo> {
    let funcs = cmd_json(core, &format!("afij @ 0x{:x}", addr))?.as_array()?.to_vec();
    let func = funcs.first()?;
    let func_addr = func.get("offset").or_else(|| func.get("addr")).and_then(|v| v.as_u64())?;
    Some(FunctionInfo {
        addr: func_addr,
...
pub unsafe fn get_function_blocks(core: *mut RCore, addr: u64) -> Vec<BasicBlockInfo> {
    ...
    let Some(json) = cmd_json(core, &format!("afbj @ 0x{:x}", addr)) else { return blocks };
    for block in json.as_array().unwrap_or(&Vec::new()) {
        if let (Some(addr), Some(size)) = (block.get("addr").and_then(|v| v.as_u64()), block.get("size").and_then(|v| v.as_u64())) {
...
pub unsafe fn cache_function_disassembly(core: *mut RCore, fcn_addr: u64) -> Option<FunctionDisassembly> {
    ...
    for bb in bbs {
        ...
        for op in ops {
            let Some(bytes_hex) = op.get("bytes").and_then(|v| v.as_str()) else { continue };
            if bytes_hex.is_empty() { continue; }
            let Ok(bytes) = hex::decode(bytes_hex) else { continue };
            ...
            instructions.push(InstructionInfo {
                ...
                op_type: op.get("type_num").or_else(|| op.get("type")).and_then(|v| v.as_u64()).unwrap_or(0) as u32,
        ...
        if !instructions.is_empty() {
            blocks.push(BlockDisassembly { addr: bb_addr, instructions });
        }
    }

    if blocks.is_empty() { None } else { Some(FunctionDisassembly { blocks }) }
}
...
unsafe fn parse_regions_from_json(json: &serde_json::Value) -> Vec<RelocatableRegion> {
    ...
    for entry in json.as_array().unwrap_or(&Vec::new()) {
        if let (Some(vaddr), Some(size)) = (
            ...
            entry.get("vsize").and_then(|v| v.as_u64()).or_else(|| entry.get("size").and_then(|v| v.as_u64())),
        ) {
            regions.push(RelocatableRegion { start: vaddr, end: vaddr + size });
        }
    }
    regions
}
...
pub unsafe fn get_relocatable_regions(core: *mut RCore) -> Vec<RelocatableRegion> {
    ...
    if let Some(json) = cmd_json(core, "iSSj") {
        ...
        let valid: Vec<_> = arr.iter().filter(|seg| {
    ...
    cmd_json(core, "iSj").map(|j| parse_regions_from_json(&j)).unwrap_or_default()
}
...
pub unsafe fn get_arch_info(core: *mut RCore) -> (String, String) {
    ...
    let Some(bin) = json.get("bin") else { return ("unknown".to_string(), "unknown".to_string()) };
    let r2_arch = bin.get("arch").and_then(|v| v.as_str()).unwrap_or("unknown");
...
pub unsafe fn is_interactive(core: *mut RCore) -> bool {
    ...
    || {
        ...
        let value = std::ffi::CStr::from_ptr(result).to_string_lossy().into_owned();
        free(result as *mut _);
        value.trim() == "true"
    }
}
...
pub unsafe fn apply_function_metadata(
    ...
    for comment in &warp_func.comments {
        ...
        let cmd = CString::new(format!("CC {} @ 0x{:x}", text, addr + comment.offset as u64)).unwrap();
        r_core_cmd(core, cmd.as_ptr());
    }
    true
}
...
fn is_nop_from_analysis(op: &serde_json::Value, bytes: &[u8]) -> bool {
    matches!(bytes, [0x90] | [0x66, 0x90] | [0x87, 0xc0])
        || bytes.len() == 4 && matches!(bytes, [0x1f, 0x20, 0x03, 0xd5] | [0x00, 0xf0, 0x01, 0xf0] | [0xbf, 0x00, 0xbf, 0x00])
        || op.get("type").and_then(|v| v.as_str()) == Some("nop")
}

fn parse_self_move_operands(disasm: &str) -> Option<(&str, &str)> {
    let operands = disasm.strip_prefix("mov ").or_else(|| disasm.strip_prefix("mov.w "))?;
...
fn is_self_move_from_analysis(op: &serde_json::Value, bytes: &[u8], arch_bits: u32) -> bool {
    ...
    if !is_self_move_mnemonic(disasm) { return false; }
...
fn is_32bit_register(reg: &str) -> bool {
    matches!(reg, "eax" | "ebx" | "ecx" | "edx" | "esi" | "edi" | "ebp" | "esp"
        | "r8d" | "r9d" | "r10d" | "r11d" | "r12d" | "r13d" | "r14d" | "r15d")
}

mod hex {
    pub fn decode(s: &str) -> Result<Vec<u8>, &'static str> {
        if s.len() % 2 != 0 { return Err("Invalid hex string length"); }
